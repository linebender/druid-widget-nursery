...
use druid::{Data, Widget, EventCtx, LifeCycle, PaintCtx, BoxConstraints, LifeCycleCtx, Size, LayoutCtx, Event, Env, UpdateCtx};
use std::ops::Range;
use druid::im::Vector;
...
impl<D: Data, T: Data, I: ListIter<T>> ListFilter<D, T, I> {
    pub fn new(inner: impl Widget<FilterIter<I>> + 'static, filter: impl Fn(&T, &D) -> bool + 'static) -> Self {
        Self {
            accepted: Vector::new(),
            filter_update: Box::new(move |indices, mut insert_index, elements, update_range, filter_option|{
                elements.for_each(|element, index|{
                    if index >= update_range.start && index < update_range.end && filter(element, filter_option) {
...
impl<T: Data, D: Data, I: ListIter<T>> Widget<(I, D)> for ListFilter<D, T, I> {
    ...
    fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, event: &LifeCycle, data: &(I, D), env: &Env) {
        if let LifeCycle::WidgetAdded = event {
            (self.filter_update)(&mut self.accepted, 0, &data.0, 0..(data.0.data_len()), &data.1);
        }
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &(I, D), env: &Env) -> Size {
        let inner_data = FilterIter::new(data.0.clone(), self.accepted.clone());
