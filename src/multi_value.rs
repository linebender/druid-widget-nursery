use druid::{WidgetPod, Widget, LifeCycle, EventCtx, PaintCtx, BoxConstraints, LifeCycleCtx, Size, LayoutCtx, Event, Env, UpdateCtx, Data, Point, Vec2};
use druid::widget::{Radio, Checkbox};
use std::fmt::Debug;
use crate::partial::{Prism, PartialWidget, OptionSome};
...
impl<W, T, U, P> MultiRadio<W, T, U, P> where
    T: Data,
    U: Data,
    P: Prism<T, U>,
    W: Widget<U>
{
    ...
    pub fn set_show_when_disabled(&mut self, show_when_disabled: bool) {
        self.inner.widget_mut().set_show_when_disabled(show_when_disabled);
    }
...
impl<W, U, T, P> Widget<T> for MultiRadio<W, T, U, P> where
    T: Data,
    U: Data + Debug,
    P: Prism<T, U>,
    W: Widget<U>
{
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &T, env: &Env) -> Size {
        ...
        self.radio.set_origin(ctx, &self.is_enabled(), env, Point::ZERO);
        ...
        self.inner.set_origin(ctx, data, env, inner_origin.to_point());
...
pub struct MultiCheckbox<W, T> where {
...
impl<W, T> MultiCheckbox<W, T> where
    T: Data,
    W: Widget<T>
{
...
impl<W, T> Widget<Option<T>> for MultiCheckbox<W, T> where
    T: Data,
    W: Widget<T>
{
    ...
    fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, event: &LifeCycle, data: &Option<T>, env: &Env) {
        self.inner.lifecycle(ctx, event, data, env);
        self.check_box.lifecycle(ctx, event, &self.is_enabled(), env);
    }
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &Option<T>, env: &Env) -> Size {
        ...
        self.check_box.set_origin(ctx, &self.is_enabled(), env, Point::ZERO);
